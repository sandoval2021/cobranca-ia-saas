...
function adminPanel(ctx){ if(ctx.role!=='super_admin') throw new AppError('Sem permissão',403); const instances=db.wa.instances; const connected=instances.filter(i=>i.is_connected).length; const disconnected=instances.filter(i=>!i.is_connected).length; const paused=instances.filter(i=>i.status==='pausado').length; const totalMsgs=db.wa.messages.length; const spamRisk=instances.filter(i=>i.sent_today>i.daily_limit*0.9).length; return {connected,disconnected,paused,total_msgs:totalMsgs,spam_risk_companies:spamRisk}; }

module.exports={createInstance,reconnect,disconnect,removeInstance,status,qr,enqueueSend,processQueue,receiveWebhook,ownerPanel,adminPanel};
