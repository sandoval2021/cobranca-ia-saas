...
async function whatsappRouter(req,res){const ctx=getCtx(req);
 ...
 if(req.method==='POST'&&req.url==='/whatsapp/fila/pausar'){const r=await svc.pauseQueue(ctx);res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify(r));}
 if(req.method==='POST'&&req.url==='/whatsapp/fila/retomar'){const r=await svc.resumeQueue(ctx);res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify(r));}
 if(req.method==='GET'&&req.url==='/whatsapp/operacional/historico'){const r=await svc.operationalHistory(ctx);res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify(r));}
 if(req.method==='GET'&&req.url==='/whatsapp/operacional/fila'){const r=await svc.queueSnapshot(ctx);res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify(r));}
